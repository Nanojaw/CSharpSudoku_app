...
public class SudokuSolver
{
    ...
    /// </summary>
    private readonly List<int> TheIndexesOfFilledCells = new();
    ...
    private void InitializeBlackList()
    {
        ...
        for (var index = 0; index < BlackListsOfCells.Capacity; index++) BlackListsOfCells.Add(new List<int>());
    }
    ...
    public bool SolveThePuzzle(bool UseRandomGenerator = true)
    {
        ...
        while (currentCellIndex < SudokuBoard.TOTAL_CELLS)
        {
            ...
            var currentCell = SudokuBoard.GetCell(currentCellIndex);
    ...
    public bool CheckTableStateIsValid(bool ignoreEmptyCells = false)
    {
        return SudokuBoard.Cells
                          ...
                          .FirstOrDefault(cell => cell.Value != -1 && !IsValidValueForTheCell(cell.Value, cell)) ==
               null;
    }
    ...
    public bool IsValidValueForTheCell(int val, Cell cell)
    {
        ...
        if (SudokuBoard.Cells.Where(c => c.Index != cell.Index && c.GroupNo == cell.GroupNo)
                       .FirstOrDefault(c2 => c2.Value == val) != null)
            return false;

        // Check the value whether exists in the row.
        if (SudokuBoard.Cells.Where(c => c.Index != cell.Index && c.Position.Row == cell.Position.Row)
                       .FirstOrDefault(c2 => c2.Value == val) != null)
            return false;

        // Check the value whether exists in the column.
        if (SudokuBoard.Cells.Where(c => c.Index != cell.Index && c.Position.Column == cell.Position.Column)
                       .FirstOrDefault(c2 => c2.Value == val) != null)
            return false;
    ...
    private int BacktrackTo(int index)
    {
        ...
        var backTrackedCell = SudokuBoard.GetCell(index);
    ...
    private int GetValidNumberForTheCell(int cellIndex, bool useRandomFactor)
    {
        ...
        if (validNumbers.Length > 0)
        {
            ...
            var choosenIndex = useRandomFactor ? Random.Next(validNumbers.Length) : 0;
        ...
        do
        {
            var currentCell = SudokuBoard.GetCell(cellIndex);
            ...
            if (theFoundValidNumber != 0 &&
                !SudokuBoard.Solver.IsValidValueForTheCell(theFoundValidNumber, currentCell))
    ...
    {
        for (var index = startCleaningFromThisIndex; index < BlackListsOfCells.Count; index++)
