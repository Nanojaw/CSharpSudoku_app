...
{
    private static readonly Dictionary<int, int> ToSquare = new()
    {
    ...
    public static List<int> GenerateSudoku(double toBeFilled)
    {
        ...
        for (var i = 0; i < numOfDeletions; i++)
        {
            var cellIndex = randomNumberGenerator.Next(0, 80);

            while (sudokuBoard.GetCell(cellIndex).Value == -1) cellIndex = randomNumberGenerator.Next(0, 80);
        ...
        while (!sudokuBoard.Solver.CheckTableStateIsValid())
        {
            ...
            for (var j = 0; j < numOfDeletions; j++)
            {
                var cellIndex = randomNumberGenerator.Next(0, 80);
    ...
    public static bool Fits(List<int> sudoku, int number, int index)
    {
        // Horizontal check
        for (var i = 0; i < 9; i++)
            if (sudoku[i + index / 9 * 9] == number)
                return false;

        for (var i = 0; i < 9; i++)
            if (sudoku[i * 9 + index % 9] == number)
                return false;
